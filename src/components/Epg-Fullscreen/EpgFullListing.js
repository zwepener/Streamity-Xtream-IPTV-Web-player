...
const EpgFullListing = () => {
  ...
  const day = date
    ? parseInt((new Date(date) - Date.now()) / (1000 * 60 * 60 * 24), 10) + 1
    : 0;

  const executeScroll = () =>
    day === 0 && scrollRef.current.scroll(new Date().getHours() * 230, 0);
  ...
  useEffect(() => {
    playingChannel &&
      listRef.current.scrollToItem(
        channelsPlaylist.findIndex(
          (x) => x.stream_id === playingChannel.stream_id,
        ),
        "center",
      );
    executeScroll();
    setTimeout(() => {
      let toSelect =
        ...
        document.querySelector(".noActive");
  ...
  const centerLogo = (event) => {
    ...
    isScrolling = setTimeout(function () {
      if (Math.abs(scrollRef.current.scrollLeft - leftScroll) > 100) {
        ...
        Array.from(document.querySelectorAll(".chLogo")).forEach(
          (x) => (x.style.left = `calc(${leftScroll}px + 140px)`),
        );
      }
    }, 66);
  };
  ...
  const close = () => {
    setTimeout(
      () =>
        document.getElementById("selectedCh")
  ...
  const moveFocus = (event) => {
    ...
    if (event.keyCode === 39 && activeElement.nextSibling) {
      ...
      next =
        event.keyCode === 40 ? next.nextElementSibling : next.previousSibling;
      do {
        if (!next) break;
        if (
          next &&
          next.children.length > 1 &&
          next.children[1].children &&
          next.children[1].children.length > 0
        )
          break;
        else
          next = next =
      ...
      if (
        ...
        if (toFocus) {
          toFocus.focus();
          setTimeout(() => {
            document.getElementById(toFocus.id) &&
