...
const Programme = ({
  ...
  const [activeLength, setActiveLenght] = useState(
    liveTime === true && stop > timer && start <= timer
      ? ((timer - start) / (stop - start)) * 100
      : false,
  );
    ...
    >
      <div style={activeLength > 0 ? isActive : {}}>
        <div>
          <BodyData>
            <Title>
              {catchup > 0 ? <i className="fas fa-history pr-2"></i> : ""}
              {title}
            </Title>
            <Description>{description}</Description>
            <Time>
              {dateFormat(new Date(start), h24Format)} -{" "}
              {dateFormat(new Date(stop), h24Format)}
            </Time>
          </BodyData>
          <ProgressBar>
            <Progress
              style={{ width: activeLength ? activeLength + "%" : "0%" }}
            ></Progress>
          </ProgressBar>
        </div>
      </div>
