...
const Groups = () => {
  ...
  useEffect(() => {
    if (!groups || groups.length === 0) history.replace("/");
  }, []);

  return (
    <>
      <div
        className={`netflix-nav-rev netflix-nav-rev-black ${navbar ? "visible" : ""}`}
      >
        <div
          className={`netflix-nav-rev netflix-nav-rev-red ${navbar ? "visible" : ""}`}
        >
          <div
            className={`netflix-nav-rev netflix-nav-rev-white ${navbar ? "visible" : ""}`}
          >
            <div className="row">
              <div className="col-7">
                <Input
              ...
              <div className="col-auto">
                <button
                  className="netflix-nav-rev-btn netflix-close-btn netflix-close-btn-reverse"
                  onClick={closeBar}
                >
