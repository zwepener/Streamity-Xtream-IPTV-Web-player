...
const RowItem = ({
  ...
  return (
    ...
    >
      <ImgContainer
        style={
          !noCover ? { backgroundImage: `url(${stream_icon})` } : noCoverStyle
        }
      >
        {!noCover ? (
          <img
      ...
      {popup && !last && (
        <PopupHover
          ...
          stream_id={stream_id}
          stream_url={generateUrl("movie", stream_id, container_extension)}
          category_id={category_id}
          style={popupStyle}
          isSeries={isSeries}
          existingTmdb={existingTmdb}
        />
      )}
    </Li>
  );
