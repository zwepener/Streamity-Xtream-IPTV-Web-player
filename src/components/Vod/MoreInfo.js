...
const MoreInfo = ({ style }) => {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      (playingMode === "series"
        ? getSeriesInfo(stream.series_id, stream.name, false, stream.tmdb)
        : getVodInfo(stream.stream_id, stream.name, stream.tmdb)
      ).then((result) => {
        if (result && result.info) {
          ...
          if (info.rating) {
            ...
            for (
              let i = 0;
              i < 5 - remainder - quotient + (quotient === 1 ? 0 : 1);
              i++
            )
              push.empty.push(1);
  ...
  useEffect(() => {
    ...
    playingMode === "movie" &&
      setUrl(
        generateUrl("movie", stream.stream_id, stream.container_extension),
      );
  }, [stream]);

  const setFavoriteGlob = (event) => {
    event.stopPropagation();
    if (!!favorite) DB.del(playingMode, stream_id, true);
          ...
          <ImgHovered
            style={
              backdropPath || ImageSrc
                ? { backgroundImage: `url(${backdropPath || ImageSrc})` }
                : { display: "none" }
            }
          />
        )}
        ...
        <Body>
          ...
          <div className="row">
            <div className="col-8">
              <div style={{ display: "flex" }}>
                {Rating && (
                  <RatingDiv>
                    {Rating.full.map((x, i) => (
                      <i key={"sf-" + i} className="fas fa-star"></i>
                    ))}
                    {Rating.half && <i className="fas fa-star-half"></i>}
                    {Rating.empty.map((x, i) => (
                      <i key={"s-" + i} className="far fa-star"></i>
                    ))}
                  </RatingDiv>
                )}
                <label>
              ...
              <DescriptionDiv
                style={
                  expandDescription ? { "-webkit-line-clamp": "unset" } : {}
                }
            ...
            <div className="col-4">
              ...
              <ButtonFavorite
                type="button"
                className="btn btn-warning btn-block mb-2 mt-1 btn-info-1"
                onClick={setFavoriteGlob}
              >
                <i
                  className={`${favorite !== false ? "fas fa-star" : "far fa-star"} mr-1`}
                ></i>
              ...
              <div className="btn-group btn-block" role="group">
                {youtubeId && (
                  <ButtonSecond
                    type="button"
                    className="btn btn-block btn-info-2"
                    data-id={2}
                ...
                {url ? (
                  <ButtonSecond
                    type="button"
                    className={`btn btn-secondary btn-info-${!youtubeId ? 2 : 3}`}
                    data-id={!youtubeId ? 2 : 3}
                    href={url}
                    target="_blank"
                  >
                    <i
                      className={`fas fa-download ${!youtubeId && "mr-1"}`}
                    ></i>
                    {!youtubeId && "Download"}
                  </ButtonSecond>
                ) : (
