    ...
    index === 0 ? (
      ...
      <GroupRow
        ...
        playlist={playlist.filter(
          (s) =>
            s.category_id === categories[index - 1].category_id &&
            (searchText
  ...
  const Cell = ({ columnIndex, rowIndex, style }) => {
    if (rowIndex === 0) {
      ...
      return (
        <RowItem
          id={index}
          key={"vod" + playlist[index].stream_id}
          name={playlist[index].name}
          stream_icon={playlist[index].stream_icon}
  ...
  return (
    <div>
      <Route path="/:playingMode/">
        ...
        <Container
          style={
            categories.length < 100 && allItems
              ? blurBackground
              : { ...blurBackground, overflowY: "hidden" }
          }
        >
          {categories.length < 100 &&
            allItems &&
            randomMovie &&
            (randomMovie.stream_id || randomMovie.series_id) && (
              <BackgroundPlayer
                key={`bg-${randomMovie.stream_id || randomMovie.series_id}`}
                name={randomMovie.name}
                category_id={randomMovie.category_id}
            ...
            {allItems ? (
              window.gSTB || categories.length > 100 ? (
                <List
                  ...
                  itemSize={(index) =>
                    index === 0
                      ? convertVhToPixels("70vh")
                      : convertVhToPixels("15vw") * 1.8
                  }
                  useIsScrolling={true}
