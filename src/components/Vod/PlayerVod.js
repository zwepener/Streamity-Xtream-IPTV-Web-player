...
const PlayerVod = () => {
  ...
  const stream = useSelector((state) => state.playlist).find(
    (x) => x.stream_id === parseInt(stream_id),
  );
  ...
  useEffect(() => {
    ...
    if (stream.direct_source) setUrl(stream.direct_source);
    else {
      setUrl(
  ...
  return (
    <Container>
      <ReactNetflixPlayer
        ...
        syncDuration={(duration, percentage) =>
          setStat({ ...streamStat, start: duration, tot: parseInt(percentage) })
        }
        OnEnded={() => {
