...
const Login = ({ url }) => {
  ...
  const [m3u8, setM3u8] = useState(
    window.m3u8warning === true && !Cookies.get("m3u8_play"),
  );
  ...
  useEffect(() => {
    if (auth.isAuth()) history.push(url || "/");
    ...
    <>
        ...
        <Box onKeyDown={remoteController} onSubmit={login}>
          ...
          <h2 className="">
            <img
              ...
              style={{ maxWidth: "30vw", maxHeight: "18vh" }}
            />
          </h2>
      ...
      {m3u8 && (
        <Popup
          ...
          title={"Informations"}
          description={
            "To login use your IPTVEditor's playlist username and password, not your email.<br/>IPTVEditor Web Player can play live channels streams only in M3U8 format.<br/>The conversion will be done automatically if streams are in Xtreamcodes format (this won't affect your playlist)."
          }
