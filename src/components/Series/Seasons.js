...
const Seasons = ({ seasonData, cover }) => {
  ...
  useEffect(() => {
        ...
        episodes: seasonData[x].map((y) => {
          return {
            ...
            url:
              y.url ||
              y.direct_source ||
              generateUrl("series", y.id, y.container_extension),
    ...
    if (s.length > 0) {
      ...
      if (stat.tot > 95 || stat.tot < 3) {
        ...
        if (
          season.episodes[season.episodes.length - 1].episode_num > stat.episode
        ) {
          stat.episode++;
        } else {
  ...
  const playEpisode = (episode) => {
    const list = seasons
      .find((x) => x.id === selectedSeason)
      .episodes.map((x) => {
        return { ...x, id: x.info.id };
      });
    dispatch(setPlaylistEpisodes(list));
    history.push(
      location.pathname.replace(
        "info",
        `play/season/${selectedSeason}/episode/${episode}`,
      ),
    );
  };
  ...
  return (
    <>
          ...
          >
            {seasons.map((x) => (
              <option
                ...
                key={"season-" + x.id}
              >
                {x.name} ({x.episodes.length} episodes)
              </option>
            ))}
          </Select>
        </div>
      ...
      <Container>
        {streamStat &&
          seasons.find((x) => x.id === selectedSeason) &&
          seasons
            .find((x) => x.id === selectedSeason)
            .episodes.map((ep) => (
              <Episode
                key={"ep-" + ep.episode_num}
                ...
                percentage={
                  ep.episode_num === streamStat.episode ? streamStat.tot : false
                }
