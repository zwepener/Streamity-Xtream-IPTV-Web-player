...
const PlayerSeries = () => {
  ...
  const tvseries = useSelector((state) => state.playlist).find(
    (x) => parseInt(x.series_id) === parseInt(stream_id),
  ).name;
  ...
  useEffect(() => {
    if (streams && streams.length > 0) {
      ...
      (!stat ||
        (stat &&
          (parseInt(stat.season) !== parseInt(season) ||
            parseInt(stat.episode) !== parseInt(episode)))) &&
        (stat = {
          id: stream_id,
          season: season,
          episode: s.id,
    ...
    <Container>
      <ReactNetflixPlayer
        ...
        onClickItemListReproduction={(id, playing) => {
          ...
          history.replace(
            `/series/category/${category}/${stream_id}/play/season/${season}/episode/${next.episode_num}/`,
          );
        }}
        onEnded={() => {
          if (stream) {
