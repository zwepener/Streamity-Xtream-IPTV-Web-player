...
const ChannelEpg = ({ chId, Name, Epg, Shift, isPlaying }) => {
  ...
  const setFavoriteGlob = (event) => {
    ...
    if (!!favorite) DB.del("live", chId, true);
  ...
  return (
    <>
      <div
        className={
          /*isPlaying ?*/ "col-md-8 col-sm-6 col-xs-5 order-3" /*: "col-md-9 col-sm-8 col-xs-7 order-3"*/
        }
      >
        <HeaderChannel className="lh-1">
          <span>
            {epgNow !== false &&
              `${dateFormat(new Date(epgNow.start), h24Format)} - ${dateFormat(new Date(epgNow.end), h24Format)}`}
          </span>
        </HeaderChannel>
      ...
      <Favorite
        className={`col-md-1 text-center order-4 p-0 ${isPlaying ? "playing" : ""}`}
