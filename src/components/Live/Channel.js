...
const Channel = ({
    ...
    if (selected) {
      setTimeout(() => {
        document.getElementById("selectedCh") &&
          (document.getElementById("selectedCh").className +=
            " hvr-shutter-in-horizontal-on");
      }, 100);
    }
  }, []);
  ...
  return (
    <Li
      tabIndex={-1}
      onClick={!selected ? selectEvent : undefined}
      style={virtualScrollStyle}
      id={selected ? "selectedCh" : ""}
      className={
        selected
          ? "channel hvr-grow-shadow  hvr-shutter-in-horizontal"
          : "channel hvr-reveal hvr-grow-shadow"
      }
    >
