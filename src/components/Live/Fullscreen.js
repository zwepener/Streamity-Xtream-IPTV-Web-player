...
const FullscreenMag = ({ externalShow, cTitle, cDesc, cDuration }) => {
  ...
  const start =
    epgNow && epgNow.start ? dateFormat(new Date(epgNow.start), h24Format) : "";
  const stop =
    epgNow && epgNow.start ? dateFormat(new Date(epgNow.end), h24Format) : "";
  ...
  return (
    <MainDiv tabIndex={0} ref={mainRef}>
          ...
          <div className="col-3">
            <Logo>
              <img
                ...
                loading={"lazy"}
                /*onError={()=>{this.onerror=null;this.src='../images/nologo.png?1'}} */ align="middle"
                alt=""
              />
            </Logo>
          </div>
      ...
      <EpgInfoBox
        ref={EpgInfoBoxRef}
        style={{ bottom: "-50%", display: !epgNow.start && "none" }}
      >
        <div>
          <Title>{title}</Title>
          ...
          {epgNow.start ? (
            <ChannelEpgBar
              key={
                playingChannel &&
                playingChannel.epg_channel_id + " " + epgNow.start
              }
              start={epgNow.start}
              stop={epgNow.end}
            />
