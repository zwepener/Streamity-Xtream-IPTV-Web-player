  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      backGroundRef.current &&
        (backGroundRef.current.style.backgroundColor = "#00000085");
      gpContainerRef.current &&
        (gpContainerRef.current.style.transform = "translateX(0)");
    }, 100);
  }, []);
  ...
  return (
    <Background ref={backGroundRef}>
      <Container ref={gpContainerRef}>
        ...
        <GroupContainer>
          <ClosePopup href="#" onClick={closePopup}>
          ...
          <GroupScroller style={{ overflow: "auto" }}>
            <GroupList tabIndex={0} autoFocus>
              {groups.map((gp, id) => (
                <GroupItem
                  tabIndex={-1}
                  className={"hvr-grow-shadow group"}
