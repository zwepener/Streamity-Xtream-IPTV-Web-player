import {
  ...
  createMovieInfo,
  createEpisode,
} from "./vod-series-name-optimizer";
...
export async function getVodTmdbData(name, existingTmdb) {
  ...
  let tmdb =
    ...
    (await axios
      .get(
        `https://api.themoviedb.org/3/search/movie${language ? `?language=${language.toLowerCase()}&` : "?"}api_key=${window.tmdb}&query=${encodeURI(name)}`,
        {
          headers: {
            "user-agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.55 Safari/537.36 Edg/86.0.622.28",
          },
        },
      )
      .catch((err) => {
  ...
  if (existingTmdb || (tmdb && tmdb.data && tmdb.data.results.length > 0)) {
    ...
    tmdb = await axios
      .get(
        `https://api.themoviedb.org/3/movie/${tmdbId}${language ? `?language=${language.toLowerCase()}&` : "?"}api_key=${window.tmdb}&append_to_response=images,credits,videos`,
        {
          headers: {
            "user-agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.55 Safari/537.36 Edg/86.0.622.28",
    ...
    if (tmdb && tmdb.data && !tmdb.data.success) {
      ...
      if (
        ...
        tmdb.images.backdrops.length > 0
      ) {
      ...
      if (
        ...
        let trailer =
          tmdb.videos.results.find((x) => x.type === "Trailer") ||
          tmdb.videos.results.find((x) => x.type === "Teaser");
...
export async function getSeriesTmdbData(name, streams, existingTmdb) {
  ...
  let tmdb =
    ...
    (await axios
      .get(
        `https://api.themoviedb.org/3/search/tv${language ? `?language=${language.toLowerCase()}&` : "?"}api_key=${window.tmdb}&query=${encodeURI(name)}`,
        {
          headers: {
            "user-agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.55 Safari/537.36 Edg/86.0.622.28",
  ...
  if (existingTmdb || (tmdb && tmdb.data && tmdb.data.results.length > 0)) {
    ...
    streams &&
      streams.length > 0 &&
      (seasons = Array.from(new Set(streams.map((x) => x.season))).map(
        (x) => `",season/${x}`,
      ));
    tmdb = await axios
      .get(
        `https://api.themoviedb.org/3/tv/${tmdbId}${language ? `?language=${language.toLowerCase()}&` : "?"}api_key=${window.tmdb}&append_to_response=images,credits,videos${seasons}`,
        {
          headers: {
            "user-agent":
              "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.55 Safari/537.36 Edg/86.0.622.28",
    ...
    if (tmdb && tmdb.data && !tmdb.data.success) {
      ...
      if (tmdb.genres) info.genre = tmdb.genres.map((x) => x.name).join(", ");
      ...
      let duration_secEp = info.episode_run_time
        ? info.episode_run_time * 60
        : 0;
      let durationEp = info.episode_run_time
        ? info.episode_run_time.toString()
        : 0;

      if (streams && streams.length > 0) {
        Array.from(new Set(streams.map((x) => x.season))).forEach((season) => {
          const eps = streams
            .filter((ch) => parseInt(ch.season) === parseInt(season))
            .sort(function (a, b) {
          ...
          eps.forEach((ep) => {
            if (tmdb[`season/${season}`]) {
              ...
              let imageEp = "";
              let overview = "";
              const epData = tmdb[`season/${season}`].episodes.find(
                (x) => parseInt(x.episode_number) === parseInt(ep.episode),
              );
              if (epData) {
                titleEp = epData.name;
                ...
                imageEp = epData.still_path
                  ? `https://image.tmdb.org/t/p/original${epData.still_path}`
                  : "";
                overview = epData.overview;
                episodes[season.toString()].push(
                  createEpisode(
                    ep.direct_source || ep.url,
                    ep.id,
                    ep.season,
                    ep.episode,
                    duration_secEp,
