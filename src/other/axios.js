...
export async function post(url, params = {}, local, useProxy) {
  ...
  if (
    local === true &&
    !window.location.origin.match(/iptveditor\.com|localhost/)
  ) {
  ...
  else if (isIptveditor === false) uri = dns + uri;
  else uri = dns;
  ...
  return isIptveditor === true && !(proxyRequired || useProxy)
    ...
    : Axios.get(uri, {
        headers: {
          "User-Agent":
            "Mozilla/6.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.55 Safari/537.36 Edge/86.0.622.28",
        },
        timeout: 25000,
      }).catch((err) => {
...
export function setDns(data) {
  ...
  isIptveditor = !!data.match(
    /iptveditor\.com|xtream-ie|opop\.pro|localhost|192\.168\.178\.71\:3100/,
  );

  if (window.isDebug === 1) data = window.dns;
