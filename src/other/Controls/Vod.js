...
export const MainVodKeyboard = {
  left: (focus) => {
    if (focus.id === "btn-2") return document.getElementById("btn-1");
  ...
  right: (focus) => {
    if (focus.id === "btn-1") return document.getElementById("btn-2");
  ...
  down: (focus) => {
    ...
    if (p && p.nextElementSibling) {
      toFocus =
        p.nextElementSibling.querySelector(
          `.vod${focus.dataset.id && `[data-id='${focus.dataset.id}']`}`,
        ) || p.nextElementSibling.querySelector(".vod");
    }
  ...
  up: (focus) => {
    ...
    if (p && p.previousElementSibling) {
      toFocus =
        p.previousElementSibling.querySelector(
          `.vod${focus.dataset.id && `[data-id='${focus.dataset.id}']`}`,
        ) || p.previousElementSibling.querySelector(".vod");
...
export const CategoryVodKeyboard = {
  ...
  down: (focus) => {
    ...
    if (focus.tagName.toLowerCase() === "button") {
      ...
      let i = 0;
      do {
        if (!toFocus.nextElementSibling) break;
  ...
  up: (focus) => {
    ...
    if (!toFocus.previousElementSibling)
      ...
      do {
        if (!toFocus.previousElementSibling) break;
...
export const VodInfoKeyboard = {
  ...
  right: (focus) => {
    ...
    if (id === 2) return document.querySelector(".btn-info-3") || focus;
  },
  left: (focus) => {
    const id = parseInt(focus.dataset.id);
    if (id === 3) return document.querySelector(".btn-info-2") || focus;
  },
  searchFocus: (focus) => {
